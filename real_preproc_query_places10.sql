            ...
            (
                SELECT
                    madlib.real_agg_array_concat(
                        ARRAY[__madlib_temp_normalized_debug_nostats__.x_norm::REAL[]]) AS independent_var,
                    madlib.real_agg_array_concat(
                        ARRAY[__madlib_temp_normalized_debug_nostats__.y]) AS dependent_var,
                    (__madlib_temp_normalized_debug_nostats__.row_id%131.0)::smallint AS buffer_id,
                    count(*) AS count
                FROM __madlib_temp_normalized_debug_nostats__
                GROUP BY buffer_id
            ) b
